...
from .common import (
    process_image,
    gemini_with_file_search,
    clean_answer,
    GEMINI_MODEL,
    GEMINI_FILE_SEARCH_STORE,
)
from .field_report_pdf import build_pdf
...
async def field_report(
    ...
    try:
        ...
        try:
            kml_text = (
                json.dumps(json.loads(kml_context), ensure_ascii=False, indent=2)
                if kml_context else "Chưa có dữ liệu đối chiếu GIS/KML."
            )
        except Exception:
...
'''
        ...
        return {
            ...
            "answer": answer,
            "sources": sources,
            "engine": "Gemini Vision + File Search",
            "model": GEMINI_MODEL,
...
@app.get("/api/field-report-pdf")
def pdf_health():
    return {"success": True, "service": "field-report-pdf"}

@app.post("/api/field-report-pdf")
async def pdf(
    ...
    return Response(
        ...
        headers={"Content-Disposition": 'attachment; filename="bao-cao-hien-truong.pdf"'},
    )
